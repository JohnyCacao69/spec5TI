...
class CalcCtrl {
	...
	public function validate() {
		...
		if (! (isset ( $this->form->kwota ) && isset ( $this->form->proc ) && isset ( $this->form->rok ))) {
		...
		if ($this->form->rok == "") {
			$this->msgs->addError('Nie podano roku');
		}
	...
	public function process(){
		...
		if ($this->validate()) {
			...
			$this->result->op = $this->form->proc * $this->form->rok * 0.01;
			$this->result->reszta = $this->result->op * $this->form->kwota;
			$this->result->konc_kwota = $this->result->reszta + $this->form->kwota;
			$this->result->miesiace = $this->form->rok * 12;
			$this->result->result =$this->result->konc_kwota/$this->result->miesiace;
	...
	public function generateView(){
		...
		$smarty->display($conf->root_path.'/app/bank.html');
	}
}
